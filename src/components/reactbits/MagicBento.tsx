...
const createParticleElement = (x: number, y: number, color = DEFAULT_GLOW_COLOR) => {
  ...
  el.style.cssText = `
    ...
    box-shadow: 0 0 6px rgba(${color}, 0.8), 0 0 10px rgba(${color}, 0.4);
    pointer-events: none;
    z-index: 10;
    left: ${x}px;
    top: ${y}px;
...
const ParticleCard: React.FC<ParticleCardProps> = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Update local card glow position
...
const BentoCardGrid: React.FC<{ children: React.ReactNode; gridRef: React.Ref<HTMLDivElement>; className?: string }> = ({
  children,
  gridRef,
  className = '',
}) => (
      ...
      <BentoCardGrid gridRef={gridRef} className={className}>
        {cards.map((card, index) => {
          ...
          const cardStyle: React.CSSProperties = {
            ...
            ...(card.lightBackground ? ({ '--card-bg-light': card.lightBackground } as React.CSSProperties) : {}),
            ...(card.darkBackground ? ({ '--card-bg-dark': card.darkBackground } as React.CSSProperties) : {}),
